...
class DalecData:
    ...
    """
    def __init__(self, start_yr, end_yr, SIP_Pars, datas, ob_str=None, dat_file="../../aliceholtdata/ahdat99_13.csv", k=None):
        ...
        [data1, data2, data3, data4, data5, data6] = datas
        ...
        self.brf_data  = data5[(data5['year'] >= start_yr) & (data5['year'] < end_yr)]
        ...
        """
        Defines the spectral response curve 
        """
        ...
        """
        [Cab, Car, Cbrown, Cw, Cm, Alpha, a, b, c] = SIP_Pars
        self.Cab    = Cab#45    #chlorophyll a+b content (mug cm-2).
        self.Car    = Car#40    #carotenoids content (mug cm-2).
        self.Cbrown = Cbrown  #brown pigments concentration (unitless).
        self.Cw     = Cw   #equivalent water thickness (g cm-2 or cm).
        self.Cm     = Cm#0.5   #dry matter content (g cm-2).
        self.Ant    = 0.0   #Anthocianins concentration (mug cm-2). 
        self.Alpha  = Alpha#600   #constant for the the optimal size of the leaf scattering element   
        self.N      = 4.0
        self.Cca    = 35    
        
        self.leaf = sip_leaf(self.Cab, self.Car, self.Cbrown, self.Cw, self.Cm, self.Ant, self.Alpha, a, b, c)
        ...
        #self.tts = np.full(8760, 30.0)
        self.tto = np.full(8760, 45.0)
        self.psi = np.full(8760, 90)
        ...
        self.CIs = CIxy(self.CI_flag, self.tts)
        self.CIo = CIxy(self.CI_flag, self.tto)
        ...
        self.hemi_pars = single_hemi_initial(self.CI_flag, self.tts, self.lidf)
        self.dif_pars = single_dif_initial(self.CI_flag, self.tto, self.lidf)
        self.hemi_dif_pars = single_hemi_dif_initial(self.CI_flag, self.lidf)
        ...
        self.CIs_MDS = CIxy(self.CI_flag, self.tts_MDS)
        self.CIo_MDS = CIxy(self.CI_flag, self.tto_MDS) 
        ...
        self.hemi_pars_MDS = single_hemi_initial(self.CI_flag, self.tts_MDS, self.lidf)
        self.dif_pars_MDS = single_dif_initial(self.CI_flag, self.tto_MDS, self.lidf)
