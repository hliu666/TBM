...
@author: hliu
"""
from PhotoSynth_test  import PhotoSynth_test 
...
def calc_ebal_canopy_pars(leaf, wl, atmoMs, Ls, ebal_rtm_pars):
    ...
    return [Esun_, Esky_, fEsuno, fEskyo, fEsunt, fEskyt, A_sun_sun, A_sha_sun, A_sha_sha, epss], [Fc, Fs]
...
def cal_reflectance(SW, ci, lai):
    ...
    ebal_rtm_pars = rtm_o(leaf, soil, tts, tto, psi, lai, lidfa, ci_flag, ci_thres)
    ...
    ebal_canopy_pars, [Fc, Fs]  = calc_ebal_canopy_pars(leaf, wl, atmoMs, Ls, ebal_rtm_pars)    
    ...
    meteo_u = [APARu_leaf, Ccu, Tcu, ecu, o, p]
    meteo_h = [APARh_leaf, Cch, Tch, ech, o, p]
    
    Vcmax25 = 30
    BallBerrySlope = 8.0
    
    #bcu_rcw, bcu_Ci, Anu0 = PhotoSynth(meteo_u, [Vcmax25, BallBerrySlope])
    #bch_rcw, bch_Ci, Anh0 = PhotoSynth(meteo_h, [Vcmax25, BallBerrySlope])

    bcu_rcw, bcu_Ci, Anu =PhotoSynth_test(meteo_u)
    bch_rcw, bch_Ci, Anh =PhotoSynth_test(meteo_h)
    
    An = (Anu*Fc + Anh*(1-Fc))*lai
    
    return  bcu_rcw, bcu_Ci, An, APARu+APARh
  
def test_photosynthesis(APAR, ci, lai):
    Ta = np.array([25.0])
    ea = 40.0
    o = 209.0
    p = 970.0    
    ...
    APAR_leaf= APAR/lai
    meteo = [APAR_leaf, Cc, Tc, ea, o, p]
    
    bc_rcw, bc_Ci, An =PhotoSynth_test(meteo)
    
    An = An*lai
    
    return  bc_rcw, bc_Ci, An 
         
sw_arr = np.linspace(0.0, 1000.0, num=1000)
Au_list, Ah_list,  At_list = [], [], []
APAR_list = []
ci = 1.0

for lai in [6.0]:# [0.5, 1.0, 2.0, 4.0, 6.0]:
    Au_lai, Ah_lai, At_lai = [], [], []
    APAR_lai = []
    for sw in sw_arr:  
        Au, Ah, At, APAR = cal_reflectance(sw, ci, lai)
        #Au, Ah, At = test_photosynthesis(sw, ci, lai)
        
        Au_lai.append(Au) 
        Ah_lai.append(Ah)  
        At_lai.append(At)
        
        APAR_lai.append(APAR)
        
    Au_list.append(Au_lai) 
    Ah_list.append(Ah_lai)  
    At_list.append(At_lai)  
    
    APAR_list.append(APAR_lai) 
...
ax[0].set_title("(a) Total canopy", fontsize=def_fontsize/1.1)
ax[0].plot(APAR_list[0], Au_list[0],  linewidth=def_linewidth, label="LAI = 0.5", color="black")
#ax[0].plot(sw_arr, Au_list[1],  linewidth=def_linewidth, label="LAI = 1.0", color="red")
#ax[0].plot(sw_arr, Au_list[2],  linewidth=def_linewidth, label="LAI = 2.0", color="blue")
#ax[0].plot(sw_arr, Au_list[3],  linewidth=def_linewidth, label="LAI = 4.0", color="darkorange")
#ax[0].plot(sw_arr, Au_list[4],  linewidth=def_linewidth, label="LAI = 6.0", color="green")
...
ax[0].set_ylabel('Resistance'.format(chr(956)), fontsize=def_fontsize)
...
ax[1].set_title("(b) Total canopy", fontsize=def_fontsize/1.1)
ax[1].plot(sw_arr, Ah_list[0],  linewidth=def_linewidth, label="LAI = 0.5", color="black")
#ax[1].plot(sw_arr, Ah_list[1],  linewidth=def_linewidth, label="LAI = 1.0", color="red")
#ax[1].plot(sw_arr, Ah_list[2],  linewidth=def_linewidth, label="LAI = 2.0", color="blue")
#ax[1].plot(sw_arr, Ah_list[3],  linewidth=def_linewidth, label="LAI = 4.0", color="darkorange")
#ax[1].plot(sw_arr, Ah_list[4],  linewidth=def_linewidth, label="LAI = 6.0", color="green")
...
ax[1].set_ylabel('CO2 concentration'.format(chr(956)), fontsize=def_fontsize)
...
p1, =ax[2].plot(sw_arr, At_list[0],  linewidth=def_linewidth, label="LAI = 0.5", color="black")
#p2, =ax[2].plot(sw_arr, At_list[1],  linewidth=def_linewidth, label="LAI = 1.0", color="red")
#p3, =ax[2].plot(sw_arr, At_list[2],  linewidth=def_linewidth, label="LAI = 2.0", color="blue")
#p4, =ax[2].plot(sw_arr, At_list[3],  linewidth=def_linewidth, label="LAI = 4.0", color="darkorange")
#p5, =ax[2].plot(sw_arr, At_list[4],  linewidth=def_linewidth, label="LAI = 6.0", color="green")
...
#labels = ['LAI = 0.5', 'LAI = 1.0', "LAI = 2.0", "LAI = 4.0", "LAI = 6.0"]  
